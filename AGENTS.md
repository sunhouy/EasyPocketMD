...
1. **File Lifecycle**: localStorage (vditor\_files) → IndexedDB cache → Server (user\_files table) → sync back on app load
...
- Backend: `api/utils/auth.js` provides `verifyPassword()` (bcrypt compare)
...
- Electron: Loads from `dist/index.html`, stores userData in app.getPath('userData')/local\_files
...
| `db.sql`                    | Full schema with 15+ tables (users, file\_history, file\_shares, etc.) |
