        ...
        const requestBody = JSON.stringify({
            ...
            input: {
                messages: [
                    {
                        role: "system",
                        content: "你是一个专业的写作助手，可以帮助用户生成各种类型的文档内容、改写文本、生成PPT大纲等。请根据用户的要求提供高质量的回复。直接返回结果，不要包含解释性文字。输出约束：严禁返回HTML标签（如<div>、<span>、<p>等）或完整HTML文档；默认只返回纯文本、Markdown或JSON。若用户要求JSON，必须返回可直接解析的合法JSON对象，不要使用代码块包裹。"
                    },
                    {
                        role: "user",
