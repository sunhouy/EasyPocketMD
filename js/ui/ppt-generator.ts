...
(function(global) {
    ...
    function initPPTGeneratorEvents() {
        ...
        if (exportEndPageInput) {
            exportEndPageInput.addEventListener('input', validatePageRange);
            exportEndPageInput.addEventListener('change', validatePageRange);
        }
        ...
        if (prevBtn) {
            prevBtn.addEventListener('click', function() {
                if (pptState.currentPage > 0) {
                    selectPage(pptState.currentPage - 1, true);
                }
            });
        }
    ...
    // 构建大纲生成提示词
    function buildOutlinePrompt(topic, fileContent, userOutline, startPage, endPage) {
        var contextText = [topic || '', fileContent || '', userOutline || ''].join('\n');
        var isAcademic = detectAcademicContext(contextText);
        var pageRange = startPage && endPage ? `${startPage}-${endPage}页` : '5-30页';
        var prompt = '';

        if (pptState.source === 'current' && fileContent) {
            prompt = `请根据以下文件内容生成一个PPT大纲，页数范围：${pageRange}。
...
3. 页数控制在${pageRange}范围内，根据内容量合理分配。
4. 每页标题必须具体，不能使用”标题””内容页”等空泛词。
5. **重要：尽可能保留原文的关键信息，每页可以包含3-8个要点，确保重要内容不被遗漏。**
6. **要点应该是完整的句子或关键信息，不要过度压缩导致信息丢失。**
7. 目录页的要点应列出主要章节，不要写空话。
8. 对于数据、案例、重要观点等关键信息，必须在大纲中体现。
...
参考资料必须是正确的参考资料格式，给出准确的来源。请只输出大纲正文，不要解释。`;
            ...
            prompt = `请为主题”${topic}”生成一个详细的PPT大纲，页数范围：${pageRange}。
...
3. 页数控制在${pageRange}范围内，根据内容量合理分配。
4. 每页标题必须具体，不能使用”标题””内容页”等空泛词。
5. **重要：每页可以包含3-8个要点，确保内容充实，信息完整。**
6. **要点应该是完整的句子或关键信息，包含具体数据、案例、观点等。**
7. 目录页的要点应列出主要章节，不要写空话。
...
请只输出大纲正文，不要解释。`;
    ...
    async function generatePage(index) {
            ...
            try {
                ...
                if (keywordsData.code === 200 && keywordsData.data) {
                    ...
                    if (imagesData.code === 200 && imagesData.data && imagesData.data.length > 0) {
                        ...
                        if (imageBase64) {
                            // 验证图片数据大小
                            var base64Content = imageBase64.split(',')[1];
                            if (base64Content && base64Content.length >= 1000) {
                                ...
                                console.log('Image added to page:', Math.round(base64Content.length / 1024) + 'KB');
                            } else {
                                console.warn('Image data too small, skipping:', base64Content ? base64Content.length : 0);
                            }
                        }
                    }
                }
        ...
        var points = (page.content || []).slice(0, 8).map(function(item, idx) {
  ...
  "subtitle": "",
    ...
    async function preloadImage(imageUrl, pageIndex) {
        return new Promise(function(resolve, reject) {
            ...
            var img = new Image();
            img.onload = function() {
                console.log('Image preloaded for page', pageIndex + 1, 'size:', img.width, 'x', img.height);
                resolve();
            };
            img.onerror = function() {
                console.error('Failed to preload image for page', pageIndex + 1);
    ...
    function renderPageFromJson(pageData, scheme) {
        ...
        if ((pageData.layout === 'image-left' || pageData.layout === 'image-right') && image) {
            var imageBlock = '<div style="width:34%;height:100%;border-radius:14px;overflow:hidden;background:rgba(255,255,255,0.14);display:flex;align-items:center;justify-content:center;">'
                + '<img src="' + escapeHtml(image.url) + '" style="width:100%;height:100%;object-fit:' + (image.fit === 'cover' ? 'cover' : 'contain') + ';" onload="this.style.opacity=1" onerror="this.style.display=\'none\'" />'
                + '</div>';
    ...
    async function doDownloadPPT(customFilename) {
        ...
        if (!validatePageRange()) {
            if (global.showMessage) {
                global.showMessage('页数范围无效，请检查设置', 'error');
            }
            return;
        }
        ...
        var endIdx = endPage - 1;

        var originalText = btn.innerHTML;
        btn.disabled = true;
        btn.innerHTML = '<i class="fas fa-spinner fa-spin"></i> 准备图片...';

        try {
            // 准备请求数据 - 只包含选中范围的页面
            var selectedPages = pptState.pages.slice(startIdx, endIdx + 1);
            var selectedOutline = pptState.outline.slice(startIdx, endIdx + 1);

            // 预加载所有图片，确保完全加载
            btn.innerHTML = '<i class="fas fa-spinner fa-spin"></i> 加载图片中...';
            var imageLoadPromises = [];
            for (var i = 0; i < selectedPages.length; i++) {
                var page = selectedPages[i];
                if (page && page.image && page.image.url) {
                    imageLoadPromises.push(preloadImage(page.image.url, i));
                }
            }
            ...
            btn.innerHTML = '<i class="fas fa-spinner fa-spin"></i> 生成PPT中...';

            var exportData = {
                topic: pptState.topic || 'PPT',
                pages: selectedPages.map(function(page, idx) {
                    if (page && typeof page === 'object' && !Array.isArray(page)) {
                        return {
                            layout: page.layout || (idx === 0 ? 'cover' : 'content'),
                            role: page.role || (selectedOutline[idx] && selectedOutline[idx].role) || 'body',
                    ...
                    var title = selectedOutline[idx] ? selectedOutline[idx].title : ('第' + (startPage + idx) + '页');
                    return {
                        layout: idx === 0 ? 'cover' : 'content',
                        role: (selectedOutline[idx] && selectedOutline[idx].role) || 'body',
                        themeToken: pptState.colorScheme,
                        title: sanitizePageText(title),
                        subtitle: '',
                        bullets: [],
