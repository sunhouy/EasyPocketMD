...
bool containsNonAscii(const std::string& input) {
    for (size_t i = 0; i < input.size(); ++i) {
        if (static_cast<unsigned char>(input[i]) >= 0x80) return true;
    }
    return false;
}
...
std::pair<int, std::string> scoreCommand(const CommandItem& item, const std::string& query) {
    ...
    const bool allowContains = containsNonAscii(query);
    ...
    auto applyField = [&](const std::string& field, const std::string& name, int exactScore, int prefixScore, int containsScore) {
        ...
        if (allowContains && (field.find(query) != std::string::npos || (!compactQuery.empty() && compactField.find(compactQuery) != std::string::npos))) {
            score += containsScore;
            matchedField = name;
            matched = true;
        }
    ...
    for (size_t i = 0; i < item.aliases.size(); ++i) {
        ...
        if (allowContains && (alias.find(query) != std::string::npos || (!compactQuery.empty() && compactAlias.find(compactQuery) != std::string::npos))) {
            score += 140;
            matchedField = "alias";
            matched = true;
        }
    ...
    for (size_t i = 0; i < item.keywords.size(); ++i) {
        ...
        if (allowContains && (keyword.find(query) != std::string::npos || (!compactQuery.empty() && compactKeyword.find(compactQuery) != std::string::npos))) {
            score += 80;
            matchedField = "keyword";
            matched = true;
        }
